...
1. **リサーチャー（調査）— 2 日に 1 回**: オープン中の全 Issue を調査し、Issue コメントとして書き込む
...
```mermaid
flowchart TD
    subgraph Researcher["リサーチャー (OpenAI API)"]
        Daily["Daily Investigator<br>既存 Issue を2日ごとに調査+採点"]
        Proposer["Proposer<br>週次で新しい絵本/機能を発案"]
    end

    subgraph Creator["作成者 (OpenAI API)"]
...
3 つのエージェント役（リサーチャー / 作成者 / こども）は、**ある Issue を処理し終えたら、その Issue に自分の役のラベルを 1 つ貼る**。3 役ぶんのラベルが揃った Issue は **自動でクローズ**する。「どの役までその Issue を見たか」をラベルで可視化し、全役が見終わった backlog を機械的に畳むのが狙い。

| 役 | 処理 | 貼るラベル |
|---|---|---|
| リサーチャー（Daily） | 既存 Issue を調査・採点した（§4） | `stage:researched` |
| 作成者（Weekly） | その Issue を実装し Draft PR 化した（§6） | `stage:implemented` |
| こども（Reviewer） | 紐づく PR を所見レビューした（§7） | `stage:child-reviewed` |

- **ラベル付与は対応した役自身が行う**。各ワークフローは自分の処理が成功した直後に、対象 Issue へ自分のステージラベルを付与する（こどもは PR の `Closes #N` から対象 Issue を辿って付与する）
...
実行エンジンは **LangGraph + OpenAI API**（Python）。LLM 呼び出しは **OpenAI 公式 SDK（`openai` パッケージ）の Chat Completions** を使う。`base_url` と `model` を設定可能にし、役割ごとにモデルを差し替えられるようにする。公式の `anthropics/claude-code-action` は使わず、自前のグラフを GitHub Actions Runner 上で動かす。こどもレビュワーのみ Playwright（ブラウザ操作）を併用する。

LLM バックエンドの抽象化（OpenAI SDK・モデル差し替え）とプロンプト／ペルソナの外部化は、それぞれ §3.1 / §3.2 で定義する。**両方とも全役（リサーチャー / 作成者 / こども）に共通適用**する。
...
flowchart LR
    ...
    subgraph External["外部"]
        Anthropic["OpenAI API (openai SDK)<br>model は役割ごとに設定<br>(軽量 / 高性能 / Vision)"]
...
### 3.1 LLM バックエンド抽象化（OpenAI SDK・モデル差し替え）

LLM 呼び出しは `common/llm.py` に集約し、**OpenAI 公式 SDK（`openai` パッケージ）の Chat Completions** を 1 本だけ叩く実装にする。`base_url` と `model` を環境変数で差し替えるだけで、エンドポイントとモデルを切り替えられる。

| 環境変数 | 役割 | 既定値（例） |
|---|---|---|
| `OPENAI_BASE_URL` | OpenAI SDK の base_url | OpenAI の `/v1`（差し替え可） |
| `OPENAI_API_KEY` | API キー（Secret 登録） | — |
| `LLM_MODEL_DAILY` | リサーチャー① の model | 軽量ティア（例 `claude-haiku-4-5`） |
| `LLM_MODEL_PROPOSER` | リサーチャー② の model | 高性能ティア（例 `claude-sonnet-4-6`） |
| `LLM_MODEL_CREATOR` | 作成者 の model | 高性能ティア |
| `LLM_MODEL_CHILD` | こども の model（**Vision 必須**） | Vision 対応ティア |

- **役割ごとに model を独立設定**する。軽い調査は軽量ティア（haiku 相当）、コード生成・発案・画像評価は高性能ティア（sonnet 相当）、という設計意図（§10）を、ハードコードでなく env で表現する
- 本ドキュメント中の `claude-haiku-4-5` / `claude-sonnet-4-6` は**既定値の例**であり、`LLM_MODEL_*` で任意のモデルに差し替えられる
- Vision が必要なのはこどもレビュワー（§7）のみ。`LLM_MODEL_CHILD` には画像入力対応モデルを設定する前提

### 3.2 プロンプト／ペルソナの外部化

各役の**システムプロンプト・ペルソナ・ルーブリック文言**はコードにハードコードせず、編集可能な外部ファイルに分離する。役の振る舞い（口調・評価観点・制約）を、Python を触らずに調整できるようにするため。

- 配置: `.github/scripts/prompts/<role>/`（`system.md` / `persona.md` / `rubric.md` 等）。各役の `prompts.py` は**このファイル群を読み込むローダ**に徹し、文言自体は持たない
- 差し替え: 環境変数 `PROMPT_DIR`（既定 `prompts/`）でディレクトリごと切り替え可能。実験用ペルソナを別ディレクトリに置いて A/B できる
- **全役共通**: リサーチャー①②・作成者・こども、いずれも同じ仕組みで外部化する。「他の役の人が指示を変えたい」場合も、該当役の `prompts/<role>/` を編集するだけで済む
- §8.1 の共通システムプロンプト（プライバシー方針）は `prompts/common/privacy_system.md` として全役の先頭に必ず連結する（外部化してもこの連結は省略不可）
...
- cron: `0 0 */2 * *` （00:00 UTC・2 日ごと = **09:00 JST 隔日**）
- モデル: 軽量ティア（既定 `claude-haiku-4-5`。`LLM_MODEL_DAILY` で差し替え可、§3.1）
- 試算: 1 Issue あたり ~3K token × 7 Issue = ~21K token/回 → 1 回あたり 1 セント未満（隔日なので月コストは毎日運用の約半分）
...
- モデル: 高性能ティア（既定 `claude-sonnet-4-6`。`LLM_MODEL_PROPOSER` で差し替え可、§3.1）。発案には弱モデルより発散と妥当性の両立が要る
...
- モデル: 高性能ティア（既定 `claude-sonnet-4-6`。`LLM_MODEL_CREATOR` で差し替え可、§3.1）。コード生成にはより強い推論が必要
...
- モデル: Vision 対応ティア（既定 `claude-sonnet-4-6`。`LLM_MODEL_CHILD` で差し替え可、§3.1）。**Vision 対応必須**（スクショ画像を読む）
...
.github/
  workflows/
    daily-issue-investigation.yml       # cron 09:00 JST 隔日（2日に1回）（リサーチャー①）
  ...
  scripts/
    requirements.txt                    # langgraph, openai, PyGithub, pydantic, playwright
    README.md                           # オペレータ向け: secrets, dry-run, ローカル再現
    prompts/                            # 全役のプロンプト/ペルソナ（外部化・編集可能、§3.2）
      common/
        privacy_system.md               # 全役の先頭に必ず連結するプライバシー方針
      daily/      system.md persona.md rubric.md
      proposer/   system.md persona.md rubric.md
      creator/    system.md persona.md
      child/      system.md persona.md rubric.md
    common/
      ...
      llm.py                            # OpenAI SDK クライアント、base_url/モデル選択、トークン上限
      prompts_loader.py                 # prompts/<role>/ の system/persona/rubric を読み込む
    daily_investigator/
...
モデルは OpenAI SDK 経由で `LLM_MODEL_*`（§3.1）で差し替え可能。下表の model 列は**既定値の例**。

| 役 / ワークフロー | モデル（既定例） | 入力 (token/run) | 出力 (token/run) | 頻度 | 月コスト見積（粗） |
|---|---|---|---|---|---|
| リサーチャー① Daily | 軽量ティア（例 `claude-haiku-4-5`） | ~1.5K × 7 issue | ~1.6K × 7 issue | 2 日に 1 回 | 1 ドル未満 |
| リサーチャー② Proposer | 高性能ティア（例 `claude-sonnet-4-6`） | ~8K | ~2K | 週 1 | 1 ドル未満 |
| 作成者 Weekly | 高性能ティア（例 `claude-sonnet-4-6`） | ~45K | ~10K | 週 1 | 数ドル |
| こども Child Review | Vision ティア（例 `claude-sonnet-4-6`） | 画像数枚 + ~30K | ~3K | PR ごと（週 1 想定） | 数ドル |
...
flowchart TD
    ...
    Merge["マージ"]
    Fix["手動修正 or close"]
    Close["Issue クローズ<br>Closes 記法 or 3役完了で自動"]
...
flowchart LR
    ...
    Sys --> LLMCall["LLM 呼び出し<br>(OpenAI API)"]
