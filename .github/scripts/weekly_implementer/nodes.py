...
def _generate_system() -> str:
    # gpt-5 系は放っておくと「計画/説明」を喋りがちなので、出力契約を強く固定する。
    # 区切りブロック以外を一切出さないこと・全文を出すこと・例を明示する。
    return (
        "あなたはコード生成器です。応答は『変更後ファイル群』だけを、下記フォーマットで"
        "返します。これは機械が直接パースします。\n"
        "\n"
        "厳守事項:\n"
        "- 前置き・計画・要約・説明・箇条書き・コメント文・感想を一切書かない。\n"
        "- 各ファイルは必ず次の 1 行ヘッダから始める:\n"
        "  === path: <リポジトリ相対パス> ===\n"
        "  ヘッダの次の行から、そのファイルの『全文』を出す（差分や一部ではなく全文）。\n"
        "- 複数ファイルはヘッダを続けて並べる。\n"
        "- コードフェンス(```)で囲まない。Markdown 見出しを付けない。\n"
        "- 変更しないファイルは出力しない。\n"
        "- 呼びかけ文は必ず __NAME__ プレースホルダで書く（実名禁止）。\n"
        "\n"
        "出力フォーマットの例（この形式そのものだけを返す）:\n"
        "=== path: doubutsu/config.js ===\n"
        "export const config = { talks: ['__NAME__、\\nやっほー'] };\n"
        "=== path: doubutsu/theme.css ===\n"
        ".page { background: #fff; }\n"
    )
...
def _generate_user(
    ...
    return (
        f"# 変更計画\n{plan}\n\n"
        f"# 現在のファイル内容\n{blocks}\n\n"
        "上記の変更計画を反映した『変更後ファイル』を出力してください。\n"
        "出力は `=== path: ... ===` ブロックだけ。計画・説明・前置きは書かないこと。"
    )
