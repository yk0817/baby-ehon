...
- **音**: WebAudio API でシーン別の音階を生成（録音ファイルは持たず合成）。`doubutsu` ではタップで動物ごとの鳴き声（いぬ＝わんわん／ねこ＝にゃー／ぶた＝ぶーぶー／ぞう＝ぱおーん／パーティー＝ファンファーレ）、`dansu` では動物ごとのダンスのリズム音（うさぎ＝ぴょんぴょん／ぺんぎん＝ぺたぺた 等）を合成再生
