...
- **音**: WebAudio API でシーン別の音階を生成（録音ファイルは持たず合成）。`doubutsu` ではタップで動物ごとの鳴き声（いぬ＝わんわん／ねこ＝にゃー／ぶた＝ぶーぶー／ぞう＝ぱおーん／パーティー＝ファンファーレ）を合成再生
...
├── densha/  ├── kuruma/  ├── otenki/  ├── yorunosora/  ├── doubutsu/  └── iro/   ← それぞれ同構成
```
